        ...
        for i in range(len(opponents)):
            ...
            if history[i] == "w":
                if opponents[i] in self.wins:
                    self.melee_lvl += (5 * (((100 - (player_list.index(opponents[i])+1)) * (math.log((player_list.index(opponents[i])+1)))) / 100))
                else:
                    self.wins.append(opponents[i])
                    self.melee_lvl += (10 * (((100 - (player_list.index(opponents[i])+1)) * (math.log((player_list.index(opponents[i])+1)))) / 100))

            # For Losses
            elif history[i] == "l":
                if opponents[i] in self.losses:
                    if (opponents[i] != Plus100):
                        self.melee_lvl -= (3.5 * (((player_list.index(opponents[i])+1) * (math.log(player_list.index(opponents[i])+1)))/ 100))
                    else:
                        self.melee_lvl -= (3.5 * 100 * math.log(100) / 100)
                else:
                    self.losses.append(opponents[i])
                    if (opponents[i] != Plus100):
                        self.melee_lvl -= (7 * (((player_list.index(opponents[i])+1) * (math.log(player_list.index(opponents[i])+1)))/ 100))
                    else:
                        self.melee_lvl -= (3.5 * 100 * math.log(100) / 100)
...
def main():
    ...
    # TOURNAMENTS IN ORDER | ONLY SORT OVER MAJOR
